...
if os.path.exists('kernel/sys.c'):
    ...
    if "susfs_is_uname_spoof_buffer_set" not in content:
        ...
        # 2. 精准捕获老 uname 的逻辑，套上安全保护壳！(已修复正则缺少的括号匹配)
        pattern_old_uname = r"(SYSCALL_DEFINE1\s*\(\s*uname\s*,[\s\S]*?if \(\s*static_branch_likely\(&susfs_is_uname_spoof_buffer_set\)\s*\)\s*\n\s*)susfs_spoof_uname\(&tmp\);"
